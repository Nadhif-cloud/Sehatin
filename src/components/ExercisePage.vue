  ...
  <div class="space-y-6 lg:space-y-8">
    ...
    <div>
      ...
      <p class="mt-2 text-gray-600">
        Pilih hari untuk melihat program latihan yang telah disiapkan untuk
        Anda.
      </p>
    </div>
    ...
    <div class="relative">
      <div class="flex space-x-2 overflow-x-auto pb-3 -mb-3 scrollbar-hide">
        <button
          ...
          class="px-5 py-2.5 text-sm font-semibold rounded-full transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-[rgba(210,247,253,0.5)] whitespace-nowrap"
          :class="
    ...
    <div
      v-else-if="error"
      class="text-center py-20 bg-white p-10 rounded-2xl shadow-lg"
    >
    ...
    >
        ...
        <div class="bg-white p-6 rounded-2xl shadow-lg">
          ...
          <div class="mt-6 pt-4 border-t border-slate-200 space-y-3 text-sm">
            <div class="flex justify-between items-center text-slate-700">
              ...
              <span class="font-bold text-[rgba(0,119,137,1)]"
                >{{ currentPlan.workout_plan_items.length }} Jenis</span
              >
            </div>
            <div class="flex justify-between items-center text-slate-700">
              <span class="font-medium">Fokus Hari Ini:</span>
              <span
                class="font-bold px-2 py-1 bg-cyan-100 text-cyan-800 rounded"
                >{{ getFocusOfTheDay() }}</span
              >
            </div>
          </div>
        </div>
      ...
      <div class="lg:col-span-2 space-y-4">
        ...
        <router-link
          ...
          class="bg-white rounded-2xl shadow-lg flex items-start gap-5 p-5 border-l-4 border-transparent hover:border-[#00c6d8] hover:shadow-xl transition-all duration-200"
        >
          <!-- Gambar -->
          <img
          ...
          <!-- Detail & Target -->
          <div
            class="flex-grow flex flex-col sm:flex-row justify-between gap-4"
          >
            <div>
            ...
            <div class="bg-slate-100 p-3 rounded-lg text-center flex-shrink-0">
              <div class="font-bold text-lg text-[rgba(0,119,137,1)]">
                {{ item.sets }}
              </div>
    ...
    <div v-else class="text-center py-20 bg-white p-10 rounded-2xl shadow-lg">
      ...
      <p class="text-gray-500 mt-2">
        Tidak ada program latihan yang dijadwalkan untuk hari ini. Nikmati waktu
        istirahat Anda.
      </p>
    </div>
  </div>
...
async function fetchWorkoutPlan(dayOfWeek) {
  ...
  try {
    const { data, error: fetchError } = await supabase
      ...
      .order("order", { foreignTable: "workout_plan_items", ascending: true })
      .single();
...
const getFocusOfTheDay = () => {
  if (!currentPlan.value || !currentPlan.value.workout_plan_items)
    return "Campuran";
  const categories = currentPlan.value.workout_plan_items.map(
    (item) => item.exercises.category
  );
