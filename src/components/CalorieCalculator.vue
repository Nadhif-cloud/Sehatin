    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
      ...
      <div class="lg:col-span-2 bg-white p-6 lg:p-8 rounded-2xl shadow-lg">
        ...
        <form @submit.prevent="calculateCalories" class="space-y-5">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <div class="grid grid-cols-2 gap-2">
                  <button type="button" @click="gender = 'Pria'" :class="gender === 'Pria' ? 'bg-gradient-to-r from-cyan-600 to-cyan-500 text-white shadow-md' : 'bg-slate-100 hover:bg-slate-200 text-slate-700'" class="py-3 font-medium rounded-lg transition-colors">Pria</button>
                  <button type="button" @click="gender = 'Wanita'" :class="gender === 'Wanita' ? 'bg-gradient-to-r from-cyan-600 to-cyan-500 text-white shadow-md' : 'bg-slate-100 hover:bg-slate-200 text-slate-700'" class="py-3 font-medium rounded-lg transition-colors">Wanita</button>
              </div>
            </div>
          </div>
          <!-- Input Berat & Tinggi -->
          ...
          <div class="pt-2">
            <button type="submit" :disabled="loading" class="w-full bg-gradient-to-r from-cyan-600 to-cyan-500 hover:from-cyan-700 hover:to-cyan-600 text-white px-6 py-3.5 rounded-xl font-semibold text-base transition-all duration-200 hover:shadow-lg disabled:opacity-60 disabled:cursor-wait">
