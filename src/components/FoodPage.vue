  ...
  <div class="space-y-6 lg:space-y-8">
    ...
    <div class="bg-white p-5 lg:p-6 rounded-2xl shadow-lg">
      <h1 class="text-2xl lg:text-3xl font-bold text-gray-800 mb-2">
        Inspirasi Makanan Sehat
      </h1>
      <p class="text-gray-500 mb-6">
        Jelajahi beragam resep lezat dan bergizi untuk mendukung gaya hidup
        sehat Anda.
      </p>
        ...
        <div>
          ...
          <div class="flex flex-wrap gap-2">
            <button
              ...
              "
              class="px-4 py-2 text-sm font-medium rounded-lg transition-colors duration-150 focus:outline-none focus:ring-1 focus:ring-offset-1 focus:ring-[rgba(201,201,201,0.5)]"
            >
              Semua
            </button>
            <button
              ...
              :class="
                selectedCategory === category
                  ...
                  : 'bg-white text-slate-700 hover:bg-slate-100 shadow-sm'
              "
              class="px-4 py-2 text-sm font-medium rounded-lg transition-colors duration-150 focus:outline-none focus:ring-1 focus:ring-offset-1 focus:ring-[rgba(180,180,180,0.5)]"
            >
              {{ category }}
            </button>
          </div>
    ...
    <div
      v-else-if="error"
      class="text-center bg-white p-10 rounded-2xl shadow-lg"
    >
      ...
      <p class="text-sm text-gray-500 mt-1">
        Terjadi kesalahan saat mengambil data resep. Silakan coba lagi nanti.
      </p>
      <pre
        class="mt-4 text-xs text-left bg-gray-100 p-2 rounded text-red-800"
        >{{ error }}</pre
      >
    </div>
    ...
    >
      <div
        ...
        :key="recipe.id"
        class="bg-white rounded-2xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-1.5 group relative"
      >
        <!-- Tombol Favorit -->
        <button
          @click.stop.prevent="toggleFavorite(recipe)"
          class="absolute top-4 right-4 bg-white/70 backdrop-blur-sm w-10 h-10 rounded-full flex items-center justify-center z-10 transition-all duration-200 hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[rgba(0,119,137,1)]"
          :class="
            recipe.is_favorited
              ? 'text-red-500'
        ...
        <router-link
          :to="{ name: 'FoodDetail', params: { id: recipe.id } }"
        ...
        >
          ...
          <div class="p-5">
            ...
            <h3
              class="text-lg font-bold text-gray-800 mb-2 truncate group-hover:text-[rgba(0,119,137,1)] transition-colors"
            >
              {{ recipe.name }}
            </h3>
            ...
            <div class="flex justify-between items-center border-t pt-4">
              ...
              <span
                class="text-sm font-semibold text-[rgba(0,119,137,1)] group-hover:text-[rgba(0,90,105,1)] transition-colors duration-200"
              >
                Lihat Detail
                <span
                  class="opacity-0 group-hover:opacity-100 group-hover:ml-1 transition-all"
                  >&rarr;</span
                >
              </span>
            </div>
